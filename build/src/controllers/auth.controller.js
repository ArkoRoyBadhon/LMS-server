...
const register = (0, HandleCatchAsync_1.default)(async (req, res) => {
    const body = req.body;
    const { newUser, accessToken, refreshToken } = await auth_service_1.default.registerUser(body);
...
const login = (0, HandleCatchAsync_1.default)(async (req, res) => {
    ...
    const { userData, accessToken, refreshToken } = await auth_service_1.default.loginUser(email, password);
    res.cookie('accessToken', accessToken, {
        sameSite: 'none',
        maxAge: 1000 * 60 * 30,
        httpOnly: true,
