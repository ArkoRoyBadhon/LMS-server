    ...
    if (ee && ee.accessibleVideos.length === 0) {
      await enrollment_model_1.Enrollment.findByIdAndUpdate(
        id,
        {
          ...
          nextVideoToUnlock:
            ee.course?.modules[0]?.lectures[ee.accessibleVideos.length + 1]
              ?._id,
        },
        { new: true },
      )
    }
    const enrollment = await enrollment_model_1.Enrollment.findById(
      id,
    ).populate({
      path: 'course',
      select: 'title description  isPublished',
...
const getEnrollmentByUserId = (0, HandleCatchAsync_1.default)(
  async (req, res) => {
    const id = req.params.id
    if (!id) {
      throw new AppError_1.default(400, 'User id is required')
