...
exports.uploadMultipleFiles = exports.replaceFile = exports.uploadFile = exports.removeFile = void 0;
...
const uploadFile = async (req, res, next) => {
    try {
        ...
        if (!isValidFileType(fileType)) {
            throw new errorhandler_1.default('Unsupported file type. Only images and PDFs are allowed.', 400);
        }
        ...
        const uploadResult = await cloudinary_1.default.v2.uploader.upload(req.file.path, {
...
const uploadMultipleFiles = async (req, res, next) => {
    try {
        ...
        if (!req.files || Array.isArray(req.files) === false) {
            throw new errorhandler_1.default('No files uploaded or invalid files array', 400);
        }
        ...
        const uploadPromises = req.files.map(async (file) => {
            ...
            const uploadResult = await cloudinary_1.default.v2.uploader.upload(file.path, {
                folder: process.env.CN_Folder,
                public_id: publicId,
                resource_type: fileType === 'application/pdf' ? 'raw' : 'image',
            });
