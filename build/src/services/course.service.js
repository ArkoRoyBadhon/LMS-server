...
const getCourseService = async id => {
  if (!id) throw new AppError_1.default(400, 'Course id is required')
  ...
  try {
    await session.withTransaction(async () => {
      const courseData = await course_model_1.Course.findById(id)
        ...
        .session(session)
      if (!courseData) throw new AppError_1.default(404, 'Course not found')
      ...
      if (moduleIds.length > 0) {
        await module_model_1.Module.deleteMany({
          _id: { $in: moduleIds },
        }).session(session)
      }
