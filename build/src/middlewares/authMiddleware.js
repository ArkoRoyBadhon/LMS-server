...
const isAuthenticated = (0, HandleCatchAsync_1.default)(async (req, res, next) => {
    const accessToken = req.cookies.accessToken;
    if (accessToken) {
        try {
            const decoded = jsonwebtoken_1.default.verify(accessToken, envConfig_1.default.ACCESS_TOKEN.SECRET);
            req.user = {
                _id: decoded.id,
                email: decoded.email,
                role: decoded.role,
    ...
    if (!refreshToken) {
        throw new AppError_1.default(401, 'Unauthorized - No valid tokens provided');
    }
    try {
        const decrypted = jsonwebtoken_1.default.verify(refreshToken, envConfig_1.default.REFRESH_TOKEN.SECRET);
